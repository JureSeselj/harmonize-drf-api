...
class FollowerDetailViewTests(APITestCase):
    def setUp(self):
        ...
        danny = User.objects.create_user(username='danny', password='password')

        Follower.objects.create(owner=jure, followed_id=2)  # 'id':1
        Follower.objects.create(owner=rosa, followed_id=3)  # 'id':2
